...
WebDriverMob.prototype.setup = function () {
    ...
    this.startBrowserMob(this._bmPort)
        ...
        .timeout(25000, "Unable to Start SeleniumServer")
        .then(function () {
...
WebDriverMob.prototype.startBrowserMob = function (port) {
    ...
    //TODO : fs.existsSynch not working , need to debug this later
    //console.log(fs.existsSynch("./browserMob/bin/browsermob-proxy") );

    bmPath = "./node_modules/webdrivermob/browserMob/bin/browsermob-proxy";
    //bmPath = "./browserMob/bin/browsermob-proxy";

    this._browserMob = spawn("sh", [bmPath]);
...
WebDriverMob.prototype.startSeleniumServer = function () {
    ...
    seleniumServer.install({
        ...
        progressCb: function(totalLength, progressLength, chunkLength) {
            console.log("Selenium setup : " + percentage + "% done");
            percentage = Math.round((progressLength/totalLength) * 10000)/100;
        }
    },function(err){
        seleniumServer.start({},function(err,child){
            self._selenium = child;
            defer.resolve();
        })
